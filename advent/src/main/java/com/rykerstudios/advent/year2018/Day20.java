...
public class Day20 extends Day<Integer> {
	...
	public int buildMap(final String regex) {
		int doors = 0;
		final Set<String> paths = expand(regex);
		log.debug(paths.toString());
		// TODO build map
		doors = 0;
		// TODO count the doors
		return doors;
	}
	...
	protected Set<String> expand(final String s) {
		final Set<String> paths = new HashSet<String>();
		if (s.startsWith("^")) {
			return expand(s.substring(1, s.length() - 1));
		}
		final int leftParen = s.indexOf("(");
		final int rightParen = s.indexOf(")");
		final int lastRightParen = s.lastIndexOf(")");
		...
		// (()|)
		// (|())
		if (leftParen > -1 && (pipe == -1 || pipe > leftParen)) {
			if (lastRightParen == -1) {
				throw new IllegalArgumentException("Mismatched parentheses in expression.");
			}
			final String s1 = s.substring(0, leftParen);
			final String s2 = s.substring(leftParen + 1, lastRightParen);
			final String s3 = s.substring(lastRightParen + 1);
			final Set<String> set1 = expand(s1);
			final Set<String> set2 = expand(s2);
			final Set<String> set3 = expand(s3);
			paths.addAll(combine(combine(set1, set2), set3));
		} else if (pipe > -1 && leftParen > -1) {
			final String s1 = s.substring(0, pipe);
			final String s2 = s.substring(pipe + 1, leftParen);
			final String s3 = s.substring(leftParen + 1);
			final Set<String> set1 = expand(s1);
			final Set<String> set2 = expand(s2);
			set1.addAll(set2);
			final Set<String> set3 = expand(s3);
			paths.addAll(combine(set1, set3));
		} else if (pipe > -1) {
			final String s1 = s.substring(0, pipe);
			final String s2 = s.substring(pipe + 1);
			final Set<String> set1 = expand(s1);
			final Set<String> set2 = expand(s2);
			paths.addAll(set1);
			paths.addAll(set2);
	...
	private Set<String> combine(final Set<String> a, final Set<String> b) {
		...
		for (final String sa : a) {
			for (final String sb : b) {
				c.add(sa + sb);
			}
		}
		return c;
