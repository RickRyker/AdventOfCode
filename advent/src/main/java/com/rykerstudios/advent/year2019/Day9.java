...
public class Day9 extends Day<Long> {
	...
	// runs on passes some checks to demonstrate it is a complete Intcode computer.
	...
	// in position mode: the parameter is interpreted as a position.
	// Like position mode, parameters in relative mode can be read from or
	// written to.

	// The important difference is that relative mode parameters don't count
	// from address 0. Instead, they count from a value called the relative base.
	...
	// (and the associated parameter modes) that seem to be functioning
	...
	@Override
	public Long part1() {
		final long[] memory = super.toLongArray(getContent());
