...
public class Day5 extends Day<Integer> {
	...
	// as a position - if the parameter is 50, its value is the value stored
	...
	// right-to-left from the opcode: the first parameter's mode is in the
	...
	// the rightmost two digits of the first value, 02, indicate opcode 2,
	...
	public Integer part1() {
		...
		computer.addInput(1);
		computer.calc();
		return (int) computer.getLastOutput();
	}
	...
	// increases by the number of values in that instruction.
	// However, if the instruction modifies the instruction pointer,
	// that value is used and the instruction pointer is not automatically
	// increased.
