...
public class Day2 extends Day<Integer> {
	...
	public Day2() {
		super(1586300, 3737498);
	}

	private List<List<Long>> parseBoxDimensions(final String[] lines) {
		final List<List<Long>> dimensions = new ArrayList<List<Long>>();
		if (lines.length == 0) {
			throw new IllegalStateException("Must have at least 1 box.");
		}
		for (final String line : lines) {
			final List<Long> box = new ArrayList<Long>();
			final String[] sides = line.split("x");
			if (sides.length != 3) {
				throw new IllegalStateException("Box must have 3 sides.");
			}
			for (final String side : sides) {
				box.add(Long.parseLong(side));
			}
	...
	public Integer part1() {
		int paper = 0;
		final List<List<Long>> dimensions = parseBoxDimensions(getLines());
		for (final List<Long> box : dimensions) {
			paper += (3 * box.get(0) * box.get(1) + 2 * box.get(1) * box.get(2) + 2 * box.get(2) * box.get(0));
		}
		return paper;
	}
	...
	public Integer part2() {
		...
		final List<List<Long>> dimensions = parseBoxDimensions(getLines());
		for (final List<Long> box : dimensions) {
			ribbon += (2 * box.get(0) + 2 * box.get(1) + box.get(0) * box.get(1) * box.get(2));
		}
		return ribbon;
	}
