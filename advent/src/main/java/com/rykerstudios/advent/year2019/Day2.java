...
public class Day2 extends Day<Integer> {
	...
	public int calc(final int[] memory) {
		final IntCodeComputer program = new IntCodeComputer(memory);
		program.addInput(0);
		program.calc();
		return (int) program.getMemory()[0];
	}
	...
	// read the input values, and the third indicates the position at which
	...
	// This opcode works just like the previous, but it multiplies instead of
	// adding.
	...
	// at addresses 1 and 2, just like before.
	...
	// make sure you first reset the computer's memory to the values in the
	// program (your puzzle input) - in other words, don't reuse memory
	// from a previous attempt.

	// Find the input noun and verb that cause the program to produce the
	// output 19690720.
	// What is 100 * noun + verb? (For example, if noun=12 and verb=2,
	...
	public Integer part2() {
		...
		for (int noun = 0; noun <= 99; noun++) {
			for (int verb = 0; verb <= 99; verb++) {
				final int[] opcodes = super.toIntArray(getContent());
				opcodes[1] = noun;
				opcodes[2] = verb;
				final IntCodeComputer program = new IntCodeComputer(opcodes);
				program.addInput(0);
				program.calc();
				result = (int) program.getMemory()[0];
				if (result == 19690720) {
