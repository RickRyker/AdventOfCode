...
public class Day11 extends Day<String> {
	...
	protected int gridSerialNumber = 9221;
	protected int[][] fuelCell = new int[300][300];

	protected int getPowerLevel(final int gsn, final int x, final int y) {
		final int rackId = x + 10;
		int power = rackId * y + gsn;
		power = power * rackId;
	...
	protected int getGridPower(final int gsn, final int x, final int y) {
		return getGridPower(gsn, x, y, 3);
	}

	protected int getGridPower(final int gsn, final int x, final int y, final int sz) {
		int power = 0;
		// System.out.println("GSN:" + gsn);
		for (int j = 0; j < sz; j++) {
			for (int i = 0; i < sz; i++) {
				int fuel = getPowerLevel(gsn, x + i, y + j);
				// System.out.print("[" + (x + i) + "," + (y + j) + "]" + ":" + fuel + ", ");
				power += fuel;
			}
			// System.out.println("");
		}
	...
	public String part1() {
		...
		for (int y = 1; y < 298; y++) {
			for (int x = 1; x < 298; x++) {
				final int power = getGridPower(gsn, x, y);
				if (maxP < power) {
					maxP = power;
					maxX = x;
					maxY = y;
		...
		for (int sz = 1; sz <= 300; sz++) {
			for (int y = 1; y <= 300 - sz + 1; y++) {
				for (int x = 1; x <= 300 - sz + 1; x++) {
					final int power = getGridPower(gsn, x, y, sz);
					if (maxP < power) {
						maxP = power;
						maxX = x;
						maxY = y;
						maxS = sz;
						System.out.println(maxX + "," + maxY + "," + maxS);
					}
				}
			}
		}
