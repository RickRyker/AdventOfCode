...
public class Day10 extends Day<Long> {
	 ...
	 * adapter rated 7 (with difference of 1 and 1). The only adapter that works
	 * with the 7-jolt rated adapter is the one rated 10 jolts (difference of 3).
	 * From 10, the choices are 11 or 12; choose 11 (difference of 1) and then 12
	 * (difference of 1). After 12, only valid adapter has a rating of 15
	...
	@Override
	public Long part1() {
		final long[] adapters = getLongs();
		return getJoltageProduct(adapters);
	}

	public Long getCountAdapterPaths(final long[] input) {
		long[] adapters = ArrayUtils.add(input, 0L);
		Arrays.sort(adapters);
		adapters = ArrayUtils.add(adapters, adapters[adapters.length - 1] + 3L);
		Arrays.sort(adapters);
		final Map<Long, Long> adapterMap = new HashMap<>();
		adapterMap.put(adapters[adapters.length - 1], 1L); // the device internal adapter
		for (int i = adapters.length - 2; i >= 0; i--) {
			for (int j = i + 1; j < adapters.length && j <= i + 3; j++) {
				if (adapters[j] - adapters[i] <= 3) {
					if (adapterMap.get(adapters[i]) == null) {
						adapterMap.put(adapters[i], adapterMap.get(adapters[j]));
					} else {
						adapterMap.put(adapters[i], adapterMap.get(adapters[i]) + adapterMap.get(adapters[j]));
					}
				}
			}
		}
		return adapterMap.get(0L);
	}

	public long getJoltageProduct(final long[] adapters) {
		Arrays.sort(adapters);
		long joltage = 0;
		int joltDiff1Count = 0;
		int joltDiff3Count = 1; // because it is always highest adapter +3
		for (int i = 0; i < adapters.length; i++) {
			long adapterJoltage = adapters[i];
			if (adapterJoltage - joltage == 1) {
				joltDiff1Count++;
			}
			if (adapterJoltage - joltage == 3) {
				joltDiff3Count++;
			}
	...
	public Long part2() {
		final long[] adapters = getLongs();
		Arrays.sort(adapters);
		return getCountAdapterPaths(adapters);
	}
