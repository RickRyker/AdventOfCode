...
public class Day8 extends Day<Integer> {
	...
	protected Node parse(final String content, final int start) {
		final String[] a = content.split(" ");
		int index = start;
		final Node root = new Node();
		// System.out.print("new Node(");
		root.nodes = new Node[Integer.parseInt(a[index].trim())];
		index++;
		// System.out.print("nodes.length=" + root.nodes.length);
		root.data = new int[Integer.parseInt(a[index].trim())];
		index++;
		// System.out.println(",data.length=" + root.data.length + ")");
		for (int i = 0; i < root.nodes.length; i++) {
			root.nodes[i] = parse(content, index);
			index += root.nodes[i].getLength();
		}
		for (int i = 0; i < root.data.length; i++) {
			root.data[i] = Integer.parseInt(a[index].trim());
			index++;
	...
	public Integer part1() {
		final Node root = parse(getContent(), 0);
